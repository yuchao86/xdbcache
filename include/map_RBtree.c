...
map_iter RBtree_begin(struct interface_map_container *_rb)
{
	...
	RBnode *first = (RBnode*)rbtree_first(rb->_rb);
	CREATE_MAP_IT(begin,NULL);
	begin.node = (first == NULL ? (RBnode*)rb->_rb->nil : first);
	return begin;
}
...
map_iter RBtree_insert(struct interface_map_container *_rb,void *key,void *val)
{
	...
	if(0 == rbtree_insert(rb->_rb,(rbnode*)n))
	{
		CREATE_MAP_IT(it,n);
		return it;
	}
...
{
	RBtree_t rb = (RBtree_t)_rb;
	RBnode *n = (RBnode*)rbtree_remove(rb->_rb,key);
	if(n)
		free(n);
}
